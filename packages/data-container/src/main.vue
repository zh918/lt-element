      ...
      <div class="operator-container">
        ...
        >
          <el-dropdown
            ...
            size="small"
            split-button
            v-if="!item.classify && item.children && (!item.isShow || item.isShow())"
          >
            {{item.text}}
          ...
          >{{item.text}}</el-button>
          <el-upload
           v-else-if="item.classify === 'upload' && !item.children && (!item.isShow || item.isShow())"
            key="upload_1"
          ...
          >
            <el-button size="small" :style="{opacity: item.progress,backgroundColor:'#409EFF',color:'#fff'}" :disabled="item.progress !== 1 ? true : false">{{item.text}}</el-button>
